...
set_max_file_limit(unsigned fd_required)
{
	...
	if (setrlimit(RLIMIT_NOFILE, &limit) == -1)
		log_message(LOG_INFO, "Failed to set open file limit to %lu:%lu failed - errno %d", limit.rlim_cur, limit.rlim_max, errno);
	else if (__test_bit(LOG_DETAIL_BIT, &debug))
		log_message(LOG_INFO, "Set open file limit to %lu:%lu.", limit.rlim_cur, limit.rlim_max);
