---
title: IPsecGw Sample
hide:
  - navigation
---

Consider the following topology where 192.168.201.1 wants to reach 192.168.102.1:
<p style="text-align: center"><img src="../assets/IPsecGW.png"></p>
IPsecGW connects multiple network segments.
On the left, we see a production topology as run in live operator networks. On the right is a LAB topology used to validate IPSEC-GW operations. In the LAB topology, the IPSEC-GW configuration and operations are the same as in the production topology. The LAB topology can be used as part of a non-regression & validation process and run on a single server hosting multiple NICs (ConnectX-7 here in our example) where each NIC is a VFIO in a dedicated VM.
