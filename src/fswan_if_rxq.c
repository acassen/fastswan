...
/* Get rx_queue from BDF formatted buffer */
static int
...
static int
rxq_iface_numa(const struct interface *iface, const struct rxq_numa *numa)
{
...
static void
rxq_show_iface_queues(struct vty *vty, struct interface *iface)
{
	char cpulist[64];
	int irqs[RXQUEUE_MAX];
...
static bool
rxq_diag_iface(struct vty *vty, struct interface *iface, struct rxq_numa *numa)
{
	char cpulist[64];
	int irqs[RXQUEUE_MAX];
	uint32_t nr_rx = 0, nr_tx = 0;
 ...
 */
int
fswan_if_rxq_cpu(const struct interface *iface, int *cpu_per_queue, int max_q)
{
...
int
fswan_if_rxq_show(struct vty *vty)
{
